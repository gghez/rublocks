...
fn render_cargo_toml(manifest: &Manifest, has_migrations: bool) -> String {
    ...
    if let Some(http) = manifest.http.as_ref() {
        ...
        if !feats.is_empty() {
            ...
            // `tower` provides `ServiceBuilder` so we can stack the layers
            // in one expression.
            deps.push_str("tower = \"0.5\"\n");
            // `http` exposes header constants used by the security_headers layer.
...
fn render_main_rs(manifest: &Manifest, has_migrations: bool) -> Result<String> {
    ...
    let http_layer = manifest.http.as_ref().map(render_http_layer);
...
/// Build the `router = router.layer(...)` block from the declared HTTP
/// middleware config. Returns `None` when no layer is requested so the
/// quoted output stays empty (no spurious `tower::ServiceBuilder` import
/// and no extra noise in projects that ship pure JSON APIs).
fn render_http_layer(http: &HttpConfig) -> TokenStream {
...
mod tests {
    ...
    fn cors_wildcard_uses_any_origin() {
        ...
        let manifest = manifest_from(
            dir.path(),
            r#"{ "name": "rb_test", "http": { "cors": { "origins": ["*"] } } }"#,
        );
        let dist = dir.path().join("dist");
        emit(&manifest, dir.path(), &dist).unwrap();
        let main_rs = fs::read_to_string(dist.join("src/main.rs")).unwrap();
